...
def _observation_space(
    ...
    return gym.spaces.Box(-1.0, 1.0, shape=(ram_size + len(states) + 5,), dtype=np.float32)
...
def _observation(
    ...
    features = np.concatenate((np.asarray(ram.features(), dtype=np.float32), state, template, velocity))
    return features
